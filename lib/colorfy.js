...
(function(){
	...
	Colorfy.prototype._getNewFileContents = function( name, color ){
		...
		return new RSVP.Promise(function(resolve, reject){
			...
			jsdom.env(self.originalContents, function(err, window){
				try {
					var document = window.document;
					var svg = document.querySelector( "svg" );
					var els = svg.querySelectorAll( ":not([fill=none])" );
					var ret = {};
					Array.prototype.forEach.call(els, function(el){
						el.setAttribute( "fill", color );
					});
					els = svg.querySelectorAll( "[stroke]:not([stroke=none])" );
					Array.prototype.forEach.call(els, function(el){
						el.setAttribute( "stroke", color );
					});
					ret[ name ] = svg.outerHTML;
					resolve( ret );
				} catch( e ){
					reject( e );
				}
