...
(function(){
	...
	var _convert = function( callback ){
		var color = "",
			newFilePath = "",
			newFilename = "";
		var files = [];
		for( var name in this.colors ){
			if( this.colors.hasOwnProperty(name) ){
				...
				newFilePath = path.join( path.dirname( this.originalFilepath ), this.ofnNoExt + "-" + name + path.extname( this.originalFilepath ) );
	...
	Colorfy.prototype.convert = function( callback ){
		var self = this;
		_convert.call( this, function(err, contents){
			if( err ){
				callback( err, null );
			} else {
