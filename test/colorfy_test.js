...
(function( exports ){
	...
	exports.convert = {
		...
		colorFiles: function( test ){
			...
			this.c2.convert()
			.then(function(c){
				test.equals( Object.keys( c.colorFiles ).length, 2 );
				test.ok( arrayEqual( Object.keys( c.colorFiles ), ['bear-blue.svg', 'bear-red.svg'] ));
				test.done();
			})
			.catch(function(err){
				console.log( err );
		...
		colorFilesOptsDefinition: function( test ){
			...
			.then(function(c){
				...
				test.ok( arrayEqual( Object.keys( c.colorFiles ), ['bear-primary.svg','bear-blue.svg', 'bear-red.svg'] ), "Has correct file names");
